...
- [x] 7.3 集成测试：不可投自己、全员弃票、**默认 30s** 超时、卧底出局、两人残局卧底胜、平票加赛、终局中部榜、房主再来一局。（**现阶段不作自动化测试要求**，规则已在实现与 Vitest 用例中覆盖，验收以产品为准。）
- [x] 7.4 E2E：3 人 / 8 人、多局会话与最终 MVP。（**现阶段不作 E2E 要求**；仓库内 Playwright 烟测可留作后续扩展。）
